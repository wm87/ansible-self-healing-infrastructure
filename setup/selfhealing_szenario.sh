...
ANSIBLE_DIR="/bigdata/tmp/ansible-self-healing-infrastructure/selfhealing"
...
*)
	log "Unbekanntes Szenario: $SCENARIO"
	exit 1
	;;
esac
...
cd "$ANSIBLE_DIR" || {
	log "Fehler: Ansible-Verzeichnis '$ANSIBLE_DIR' nicht gefunden."
	exit 1
}
